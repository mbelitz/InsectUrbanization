...
blattodea_plot <- filter(mdf_scaled, Order == "Blattodea") %>% 
    ggplot() +
    geom_point(mapping = aes(x = Dev_1, y = abundance, color = scientific_name, fill = scientific_name)) +
    geom_smooth(mapping = aes(x = Dev_1, y = abundance, color = scientific_name, fill = scientific_name),
                method = "lm",
                alpha = 0.1, linewidth = 0.4, se = F) +
    geom_smooth(mapping = aes(x = Dev_1, y = abundance), method = "glm.nb", 
                 linewidth = 1) +
    ...
    labs(y = "Abundance", x = "Proportion developed (1-km)")
blattodea_plot
...
coleoptera_plot <- filter(mdf_scaled, Order == "Coleoptera") %>% 
    ...
    geom_smooth(mapping = aes(x = Dev_1, y = abundance, color = scientific_name, fill = scientific_name),
                method = "lm",
                alpha = 0.1, linewidth = 0.4, se = F) +
    geom_smooth(mapping = aes(x = Dev_1, y = abundance), method = "glm.nb", linewidth = 1) +
...
neuroptera_plot <- filter(mdf_scaled, Order == "Neuroptera") %>% 
    ...
    geom_smooth(mapping = aes(x = Dev_1, y = abundance, color = scientific_name, fill = scientific_name),
                method = "lm",
                alpha = 0.1, linewidth = 0.4, se = F) +
    geom_smooth(mapping = aes(x = Dev_1, y = abundance), method = "glm.nb", linewidth = 1, se = F)  +
    scale_color_manual(values = rep("black", 19)) +
    scale_fill_manual(values = rep("black", 19)) +
    theme_classic()+
    theme(legend.position = "none") +
...
ggplot(mdf_scaled, mapping = aes(x = Dev_1, y = abundance, color = scientific_name, fill = scientific_name)) +
    geom_point() +
    geom_smooth(mdf_scaled, 
                mapping = aes(x = Dev_1, y = abundance, 
