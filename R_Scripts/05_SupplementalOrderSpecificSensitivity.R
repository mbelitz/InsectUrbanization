...
#Script to run order-specific analysis without Hydaticus bimarginatus in the dataset
...
# tab outputs
write.csv(fixed, "Tables/Supplemental_fixedEffects_Orderspecific.csv", row.names = F)

write.csv(random, "Tables/Summplemental_randomEffects_Orderspecific.csv", row.names = F)
...
#ggsave(filename = "Figures/Supplemental_OverallResponseToUrb_OrderRE.png", dpi = 450)
...
#ggsave(filename = "Figures/Supplemental_OrderResponseToUrb.png", dpi = 450)

# combine A and B
library(ggpubr)

ga <- ggarrange(b, a, labels = c("A", "B"), ncol = 2, nrow = 1)
ggsave(filename = "Figures/Supplemental_FigureS5_OrderResponse.png",
